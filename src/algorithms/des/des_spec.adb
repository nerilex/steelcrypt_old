...
package body DES_Spec is
   ...
   procedure Permute
     (Out_Buffer :    out u8_Array;
      In_Buffer  : in     u8_Array;
      Table      : in     Permutation_Table_T)
   is
      use System;
   begin
      ...
      for i in Table.Table'Range loop
         Bit_Set
           (Out_Buffer,
            i - 1,
            Bit_Get (In_Buffer, Table.Table (i) - 1, High_Order_First),
            High_Order_First);
      end loop;
   end Permute;
   ...
   function F
     (X         : Block_32_Bit;
      Round_Key : Block_48_Bit) return Block_32_Bit
   is
   ...
   procedure RoundKey_Step
     (Key       : in out Block_56_Bit;
      Steps     : in     Positive;
   ...
   begin
      C :=
        Shift_Left
          (Shift_Left (Shift_Left (u32 (Key (1)), 8) or u32 (Key (2)), 8) or
           u32 (Key (3)),
           4) or
        u32 (Shift_Right (Key (4), 4));
      D :=
        Shift_Left
          (Shift_Left
             (Shift_Left (u32 (Key (4) and 16#0F#), 8) or u32 (Key (5)),
              8) or
           u32 (Key (6)),
           8) or
        u32 (Key (7));
      if Backwards = False then
         ...
         C :=
           C or (Shift_Right ((C and 16#C0000000#), 4) and not 16#F000_0000#);
         D :=
           D or (Shift_Right ((D and 16#C0000000#), 4) and not 16#F000_0000#);
      end if;
      Key (1) := u8 (Shift_Right (C, 20) and 16#FF#);
      Key (2) := u8 (Shift_Right (C, 12) and 16#FF#);
      Key (3) := u8 (Shift_Right (C, 4) and 16#FF#);
      Key (4) :=
        u8 (Shift_Left (C, 4) and 16#F0#) or
        u8 (Shift_Right (D, 24) and 16#0F#);
   ...
   begin
      ...
      for Round in reverse 0 .. 7 loop
         ...
         RoundKey_Step
           (Key,
            Integer (Round_Shifts (2 + 2 * Round)),
            Backwards => True);
         Permute (Round_Key, Key, PC2_Table);
         Data (5 .. 8) := Data (5 .. 8) xor F (Data (1 .. 4), Round_Key);
         RoundKey_Step
           (Key,
            Integer (Round_Shifts (1 + 2 * Round)),
            Backwards => True);
      end loop;
      Swap (Data (1 .. 4), Data (5 .. 8));
      Permute (Block, Data, FP_Table);
