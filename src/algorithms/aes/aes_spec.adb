...
package body AES_Spec is
   ...
   begin
      ...
      for i in
        1 + Key'Length / 4 .. Context.RoundKeys'Length * RoundKey_T'Length / 4
      loop
         ...
         if (i - 1) mod Nk = 0 then
            ...
            if Nk > 6 and then (i - 1) mod Nk = 4 then
               SubArray (temp);
            end if;
         end if;
      ...
      for i in Context.RoundKeys'Range loop
         Context.RoundKeys (i) :=
           RoundKey_T (w (j .. j + RoundKey_T'Length - 1));
         j := j + RoundKey_T'Length;
      end loop;
   end Initialize_Generic;

   procedure Initialize_priv is new Initialize_Generic
     (T_In  => Key_128_T,
      T_Out => Context_128_T);
   procedure Initialize_priv is new Initialize_Generic
     (T_In  => Key_192_T,
...
--           Put("  (" & Integer'Image(i) & ") t: "); print_hex(temp); New_Line;
         Block (i * 4 + 1) :=
           gf256mul (2, temp (1)) xor
           gf256mul (3, temp (2)) xor
           temp (3) xor
           temp (4);
         Block (i * 4 + 2) :=
           gf256mul (2, temp (2)) xor
           gf256mul (3, temp (3)) xor
           temp (4) xor
           temp (1);
         Block (i * 4 + 3) :=
           gf256mul (2, temp (3)) xor
           gf256mul (3, temp (4)) xor
           temp (1) xor
           temp (2);
         Block (i * 4 + 4) :=
           gf256mul (2, temp (4)) xor
           gf256mul (3, temp (1)) xor
           temp (2) xor
           temp (3);
--           Put("  (" & Integer'Image(i) & ") r: "); print_hex(Block(i * 4 + 1 .. i * 4 + 4)); New_Line;
   ...
   begin
      for i in 0 .. 3 loop
         ...
         --           Put("  (" & Integer'Image(i) & ") t: "); print_hex(temp); New_Line;
         Block (i * 4 + 1) :=
           gf256mul (14, temp (1)) xor
           gf256mul (11, temp (2)) xor
           gf256mul (13, temp (3)) xor
           gf256mul (9, temp (4));
         Block (i * 4 + 2) :=
           gf256mul (9, temp (1)) xor
           gf256mul (14, temp (2)) xor
           gf256mul (11, temp (3)) xor
           gf256mul (13, temp (4));
         Block (i * 4 + 3) :=
           gf256mul (13, temp (1)) xor
           gf256mul (9, temp (2)) xor
           gf256mul (14, temp (3)) xor
           gf256mul (11, temp (4));
         Block (i * 4 + 4) :=
           gf256mul (11, temp (1)) xor
           gf256mul (13, temp (2)) xor
           gf256mul (9, temp (3)) xor
           gf256mul (14, temp (4));
         --           Put("  (" & Integer'Image(i) & ") r: "); print_hex(Block(i * 4 + 1 .. i * 4 + 4)); New_Line;
      end loop;
   end InvMixRows;

   procedure Encrypt_Generic
...
--        Put("( 1) pre-add:   "); print_hex(Block); New_Line;
      Block :=
        u8_Array (Block) xor
        u8_Array (Context.RoundKeys (Context.RoundKeys'First));
...
--           Put("(" & Integer'Image(i) & ") pre-add:   "); print_hex(Block); New_Line;
      ...
      Block :=
        u8_Array (Block) xor
        u8_Array (Context.RoundKeys (Context.RoundKeys'Last));
   end Encrypt_Generic;
   ...
   begin
      ...
      Block :=
        ...
        u8_Array (Context.RoundKeys (Context.RoundKeys'Last));
      for i in
        reverse Context.RoundKeys'First + 1 .. Context.RoundKeys'Last - 1
      loop
         --           Put("(" & Integer'Image(i) & ") pre-shift: "); print_hex(Block); New_Line;
