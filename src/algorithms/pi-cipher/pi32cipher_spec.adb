...
package body Pi32Cipher_Spec is
   ...
   Mu_Constants : constant Phi_Constants :=
     (16#F0E8E4E2#, 16#E1D8D4D2#, 16#D1CCCAC9#, 16#C6C5C3B8#);
   Mu_Rotation_Constants : constant Phi_Rotation_Constants := (5, 11, 17, 23);
   Nu_Constants          : constant Phi_Constants          :=
     (16#B4B2B1AC#, 16#AAA9A6A5#, 16#A39C9A99#, 16#9695938E#);
   ...
   begin
      ...
      for i in Chunk'Range loop
         Ret (i) :=
           Rotate_Left
             (Constants (i) + Sum - Chunk (V_Constants (i)),
              Rotation_Constants (i));
      end loop;
      Sum := 0 xor Ret;
      Ret := Ret xor Sum;
      return Ret;
   ...
   function Nu is new Phi
     (Constants          => Nu_Constants,
      Rotation_Constants => Nu_Rotation_Constants,
      V_Constants        => Nu_V_Constants);

   function Mu (Chunk : Chunk_T) return Chunk_T is
      function foo is new Phi
        (Constants          => Mu_Constants,
         Rotation_Constants => Mu_Rotation_Constants,
         V_Constants        => Mu_V_Constants);
      Ret : Chunk_T;
   begin
      Ret := foo (Chunk);
   ...
   begin
      ...
      Scratch (Key'Length + 1 .. Key'Length + Public_Nonce'Length) :=
        Public_Nonce;
      Scratch (Key'Length + Public_Nonce'Length + 1) := 1;
      for i in Context.State'Range loop
         Context.State (i) :=
           Load_le
             (Scratch (Index .. Index + Chunk_T'Length * Word_T'Size / 8 - 1));
      ...
      for i in 1 .. 64 / Word_T'Size loop
         Context.Counter :=
           Context.Counter or
           Shift_Left (u64 (Context.State (2) (i)), (i - 1) * Word_T'Size);
      end loop;
   end Initialize;

   function "+"
     (Context      : Context_T;
      Block_Number : Block_Number_T) return State_T
   is
   ...
   begin
      for i in 1 .. 64 / Word_T'Size loop
         Ret (1) (i) :=
           Ret (1) (i) xor
           Word_T (Counter and (Shift_Left (u64'(1), Word_T'Size) - 1));
   ...
   begin
      for i in 0 .. N / 2 - 1 loop
         Store_le
           (Data (Index .. Index + 4 * Word_T'Size / 8 - 1),
            State (2 * i + 1));
   ...
   begin
      for i in 0 .. N / 2 - 1 loop
         Ret (2 * i + 1) :=
           Ret (2 * i + 1) xor
           Chunk_T'(Load_le (Data (Index .. Index + 4 * Word_T'Size / 8 - 1)));
         Index := Index + 4 * Word_T'Size / 8;
      end loop;
      return Ret;
   end "xor";
   ...
   begin
      Context.Tag :=
        Context.Tag + Extract (Pi (Pi (Context + Block_Number) xor Block));
   ...
   begin
      for i in 1 .. Block'Length / Block_Bytes loop
         Process_Header_Block
           (Context,
            Block (Index .. Index + Block_Bytes - 1),
            Num);
         Num   := Num + 1;
   ...
   begin
      ...
      Block (Index .. Block'Last) :=
        Temp_Block (Temp_Block'First .. Temp_Block'First + Block'Last - Index);
      State       := Pi (State);
      Context.Tag := Context.Tag + Extract (State);
   ...
   begin
      ...
      Block (Index .. Block'Last) :=
        Extract (State)
          (Temp_Block'First .. Temp_Block'First + Block'Last - Index);
      Temp_Block
        (Temp_Block'First + Block'Last - Index + 1 .. Temp_Block'Last) :=
        Extract (State)
          (Temp_Block'First + Block'Last - Index + 1 .. Temp_Block'Last);
   ...
   begin
      ...
      Process_Header_Last_Block
        (Context      => Ctx,
         Block        => AD,
         Block_Number => 1);
      Crypt (Crypt'First .. Crypt'First + Secret_Nonce'Length - 1) :=
        Secret_Nonce;
      Crypt (Crypt'First + Secret_Nonce'Length .. Crypt'Last - Tag_Bytes) :=
        Msg;
      Encrypt_Secret_Message_Number
        (Context => Ctx,
         Block   =>
           Crypt (Crypt'First .. Crypt'First + Secret_Nonce'Length - 1));
      Encrypt_Last_Block
   ...
   begin
      ...
      Process_Header_Last_Block
        (Context      => Ctx,
         ...
         Block_Number => 1);
      Secret_Nonce :=
        Cipher (Cipher'First .. Cipher'First + Secret_Nonce'Length - 1);
      Msg :=
        Cipher (Cipher'First + Secret_Nonce'Length .. Cipher'Last - Tag_Bytes);
      Decrypt_Secret_Message_Number (Context => Ctx, Block => Secret_Nonce);
      Decrypt_Last_Block (Context => Ctx, Block => Msg, Block_Number => 1);
      Is_Valid :=
        Pi32Cipher_Spec.Is_Valid
          (Ctx,
           Cipher (Cipher'Last - Tag_Bytes + 1 .. Cipher'Last));
   end Decrypt;
