...
package body Sha3_Generic is
   ...
   begin
      for i in 0 .. Rate_Bytes / 8 - 1 loop
         temp :=
           Load_le (Block (Block'First + i * 8 .. Block'First + i * 8 + 7));
         Context.A (x_T (i mod 5), y_T (i / 5)) :=
           Context.A (x_T (i mod 5), y_T (i / 5)) xor temp;
      end loop;
      Permute (Context.A);
   end Next_Block;
   ...
   procedure Append_Bit
     (Context : in out Context_T;
      Index   : in out Capacity_T;
      Value   : in     Bit)
   is
   begin
      Context.A (x_T ((Index / w) mod 5), y_T (Index / w / 5)) :=
        Context.A (x_T ((Index / w) mod 5), y_T (Index / w / 5)) xor
        Shift_Left (Word_T (Value), Index mod w);
      Index := Index + 1;
   ...
   begin
      ...
      for j in 0 .. Length - 1 loop
         Append_Bit
           (Context,
            Dest_Index,
            Bit (Shift_Right (Block (Index), j) mod 2));
      end loop;
      -- append suffix
      for j in 0 .. Suffix_Bit_Length - 1 loop
         Append_Bit
           (Context,
            Dest_Index,
            Bit (Shift_Right (Suffix, Natural (j)) mod 2));
      end loop;
      -- insert Padding
   ...
   begin
      for i in 0 .. Data'Length / Rate_Bytes loop
         ...
         while Length_A + 8 <= Length_B loop
            Store_le
              (Data (Length_A .. Length_A + 7),
               Context.A (x_T (j mod 5), y_T (j / 5)));
            Length_A := Length_A + 8;
            j        := j + 1;
         end loop;
         ...
         if Length_A <= Length_B then
            ...
            Data (Length_A .. Length_B) :=
              Temp (Temp'First .. Temp'First + Length_B - Length_A);
         end if;
