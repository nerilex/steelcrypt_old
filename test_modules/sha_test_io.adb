      ...
      if End_Of_File (Context.File) then
         ...
         case C is
            ...
            when 'L' =>
               next := Message_Length;
            when 'e' =>
               next := Seed; -- second char from "Seed"
            when 'C' =>
               next := Count;
            when 's' =>
               next := Message_Block; -- second char from "Msg"
            when 'D' =>
               next := Message_Digest; -- second char from "MD"
            when others =>
      ...
      loop
         ...
         case nt is
            when Sha_Test_IO.Finish =>
               ...
               Put
                 ("    (ok: " &
                  Integer'Image (ok_test) &
                  " / fail: " &
                  Integer'Image (fail_test) &
                  ")");
               New_Line;
               exit;
            when Digest_Length =>
            ...
            when Count =>
               ...
               begin
                  blob (1 + 0 * Digest_Size_Bytes .. 1 * Digest_Size_Bytes) :=
                    seed_val;
                  blob (1 + 1 * Digest_Size_Bytes .. 2 * Digest_Size_Bytes) :=
                    seed_val;
                  blob (1 + 2 * Digest_Size_Bytes .. 3 * Digest_Size_Bytes) :=
                    seed_val;
                  for i in 1 .. 1000 loop
                     Hash (blob, digest);
                     blob (1 .. 2 * Digest_Size_Bytes) :=
                       blob
                         (1 + 1 * Digest_Size_Bytes .. 3 * Digest_Size_Bytes);
                     blob
                       (1 + 2 * Digest_Size_Bytes .. 3 * Digest_Size_Bytes) :=
                       digest;
                  end loop;
                  seed_val := digest;
               end;
